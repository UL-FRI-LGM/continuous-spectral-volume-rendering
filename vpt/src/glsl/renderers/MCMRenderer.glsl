...
vec4 sampleVolumeColor(vec3 position) {
    vec2 volumeSample = texture(uVolume, position).rg; // DENSITY
    vec4 transferSample = texture(uTransferFunction, volumeSample); // TRUE EXTINCTION volumeSample.a
    return volumeSample.r;
}
...
void main() {
    Photon photon;
    vec2 mappedPosition = vPosition * 0.5 + 0.5; // 0 - 1 [x, y]
