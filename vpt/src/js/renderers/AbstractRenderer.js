...
export class AbstractRenderer extends PropertyBag {
  constructor(gl, volume, camera, environmentTexture, options = {}) {
    ...
    this._clipQuadProgram = WebGL.buildPrograms(
      gl,
      {
        quad: SHADERS.quad,
      },
  ...
  _rebuildBuffers() {
    ...
    const gl = this._gl;
    // console.log("frame buff spec:", this._getFrameBufferSpec());
    this._frameBuffer = new SingleBuffer(gl, this._getFrameBufferSpec());

    // console.log("acc buff spec:", this._getAccumulationBufferSpec());
    this._accumulationBuffer = new DoubleBuffer(
      gl,
      this._getAccumulationBufferSpec()
    );
