...
export class RenderingContext extends EventTarget {
  constructor(options = {}) {
    ...
    this.webglcontextrestoredHandler =
      this.webglcontextrestoredHandler.bind(this);
  ...
  initGL() {
    ...
    this.program = WebGL.buildPrograms(
      gl,
      {
        quad: SHADERS.quad,
      },
  ...
  async recordAnimation(options = {}) {
    ...
    if (options.type === "images") {
      ...
      const directory = await parentDirectory.getDirectoryHandle(timestamp, {
        create: true,
      });
  ...
  async recordAnimationToImageSequence(options = {}) {
    ...
    for (let i = 0; i < frames; i++) {
      ...
      this.dispatchEvent(
        new CustomEvent("animationprogress", {
          detail: (i + 1) / frames,
        })
      );
    }
